...
// ★ 핵심 기능
//
// 1. 공격할 때마다 무작위 방향을 선택
//
// 2. 동시에 여러 발을 쏘지 않고
//    탄환 1개만 발사
//
// 3. 방향을 완전히 랜덤으로 선택한 뒤,
//    그 방향으로 무조건 쏘는 것이 아니라
//    플레이어를 향하는 각도를 계산
//
// 4. 따라서 플레이어가 현재 어디에 있든
//    그 위치를 향해 탄환이 날아감
//
// 5. 공격마다 다시 계산하므로
//    다음 공격 때는 다른 방향에서 올 수 있음
//
// =========================================================
...
function bossShoot(boss) {
    ...
    // 무작위 조준 오차
    //
    // -45도 ~ +45도 사이의 랜덤한 각도를 추가
    //
    // 이렇게 하면 정확히 플레이어 위치만 따라가는
    // 것이 아니라 공격마다 방향이 조금씩 달라집니다.
    //
    // 그래도 기본적으로는 플레이어를 향합니다.
    // -----------------------------------------------------
