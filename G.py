...
function updateStage(dt) {
    ...
    if (
        ...
        if (boss) {
            ...
            // 2초마다 보스의 공격 속도 수치를 증가시킨다.
            // 중요: 이 값은 탄환 이동속도가 아니라 공격 빈도에만 사용한다.
            bossAttackSpeedTimer -= dt;

            if (bossAttackSpeedTimer <= 0) {
                bossAttackSpeedTimer += 2;

                bossAttackSpeed =
                    Math.min(1200, bossAttackSpeed + 75);
            }

            // 공격속도 수치를 실제 공격 간격으로 변환한다.
            // 250 -> 325 -> 400 ... -> 1200
            // 수치가 커질수록 공격 간격이 짧아진다.
