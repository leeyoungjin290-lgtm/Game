...
function bossShoot(boss) {
    ...
    // 후반부에는 더 공격적인 패턴이 나옵니다.
    const hpRatio = boss.hp / boss.maxHp;

    let pattern;

    if (hpRatio < 0.30 && Math.random() < 0.45) {
    ...
    // 6. 플레이어 위치 지연 폭발
    // ----------------------------------------------------
    else if (pattern === "spikeTarget") {

        // 사용하지 않는 예비 패턴
        // Ground Spike와 동일한 방식으로 확장 가능.
...
function updateBossProjectiles(dt) {
    ...
    for (
        ...
        // ------------------------------------------------
            ...
            projectile.telegraph -= dt;

            if (
                projectile.telegraph <= 0
            ) {
            ...
            if (projectile.active) {
                ...
                if (
                    along > -40 &&
                    along < projectile.length &&
                    side < projectile.width
                ) {
